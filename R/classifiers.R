CLS = setRefClass('CLS', contains = "MODEL",
    methods = list(
      initialize = function(...){
        callSuper(...)
        ...
        if(is.null(config$metric)){
          config$metric <<- chif(config$predict_probabilities, function(y_pred, y_true) {sum(y_true == y_pred)/length(y_true)}, function(y_pred, y_true) {sum(y_true == rowMax(y_pred))/length(y_true)})
        }
      },
      ...
      predict = function(X){
        ...
        if(inherits(objects$model, c('numeric', 'factor', 'integer'))){XOUT = objects$model %>% vect.extend(nrow(X)) %>% as.data.frame}
        else {XOUT = .self$model.predict(XFET)}
        if(config$predict_probabilities & config$return_logit){XOUT %<>% as.matrix %>% apply(2, function(x) log(x/(1-x))) %>% as.data.frame}
...
CLS.BIN = setRefClass('CLS.BIN', contains = "CLS",
  methods = list(
    initialize = function(...){
      callSuper(...)
      type             <<- 'Abstract Binary Classifier'
...
CLS.SCIKIT = setRefClass('CLS.SCIKIT', contains = "CLS.BIN",
   methods = list(
     initialize = function(...){
       callSuper(...)
...
CLS.FLASSO = setRefClass('CLS.FLASSO', contains = 'CLS.BIN', methods = list(
  initialize = function(...){
    callSuper(...)
...
#' @export CLS.MLR
CLS.MLR = setRefClass('CLS.MLR', contains = "CLS",
    methods = list(
      initialize = function(...){
        library(mlr)
        callSuper(...)
...
#' @export KERAS
CLS.KERAS = setRefClass('KERAS', contains = 'CLS.BIN',
  methods = c(
    initialize = function(...){
      callSuper(...)
      type <<- 'Neural Network'
...
#' @export SPARKLYR.GBT
CLS.SPARKLYR.GBT = setRefClass('SPARKLYR.GBT', contains = 'CLS.BIN', methods = list(
  initialize = function(...){
    callSuper(...)
